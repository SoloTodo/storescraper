...
from storescraper.categories import COMPUTER_CASE, \
    CPU_COOLER, GAMING_CHAIR, HEADPHONES, KEYBOARD, MONITOR, \
    POWER_SUPPLY, SOLID_STATE_DRIVE, VIDEO_CARD, KEYBOARD_MOUSE_COMBO
...
class Oltouch(StoreWithUrlExtensions):
    url_extensions = [
        ['audio', HEADPHONES],
    ...
    def discover_urls_for_url_extension(cls, url_extension, extra_args=None):
        ...
        while True:
            ...
            url_webpage = 'https://oltouch.cl/collections/{}?page={}'.format(
                              url_extension, page)
            print(url_webpage)
            ...
            for container in product_containers:
                product_url = 'https://oltouch.cl' + container.find('a')['href']
    ...
    def products_for_url(cls, url, category=None, extra_args=None):
        ...
        picture_urls = ['https:' + x['src'] for x in
                        soup.find(
                            'div', 'product-media-modal__content').findAll(
                            'img')]
        description = html_to_markdown(
            str(soup.find('div', 'product__description')))
