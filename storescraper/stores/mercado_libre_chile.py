...
class MercadoLibreChile(Store):
    @classmethod
    def categories(cls):
        return [i for i in set(cls.ml_categories_name().values()) if i]
    ...
    def get_products(cls, session, category, query_code, product_urls,
        ...
        while True:
            ...
            if not product_containers:
                if offset == 0:
                    logging.warning('Empty category: ' + categories_name[
                        categories_code[query_code]])
                break

            for container in product_containers:
                category_name = categories_code[
                    container['category_id']]
                if categories_name[category_name] != category:
                    continue
    ...
    def discover_urls_for_category(cls, category, extra_args=None):
        ...
        for category_name in categories_name.items():
            if category_name[1] == category:
                query_categories_name.append(category_name[0])

        for category_code in categories_code.items():
