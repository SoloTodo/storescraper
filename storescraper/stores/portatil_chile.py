...
class PortatilChile(Store):
    @classmethod
    def categories(cls):
        return [NOTEBOOK, VIDEO_GAME_CONSOLE]
    ...
    def discover_urls_for_category(cls, category, extra_args=None):
        ...
        for url_extension, local_category in url_extensions:
            ...
            while True:
                ...
                url_webpage = "https://portatilchile.com/{}?page={}" "".format(
                    url_extension, page
                )
                print(url_webpage)
    ...
    def products_for_url(cls, url, category=None, extra_args=None):
        ...
        if soup.find("div", {"id": "product-images-thumbs"}):
            picture_urls = [
                tag["data-image-large-src"]
                for tag in soup.find("div", {"id": "product-images-thumbs"}).findAll(
                    "img"
                )
            ]
        else:
            picture_urls = [
                soup.find("div", "images-container").find("img")["data-image-large-src"]
            ]
