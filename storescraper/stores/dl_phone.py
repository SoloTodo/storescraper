...
from storescraper.categories import CELL, HEADPHONES, NOTEBOOK, TABLET, WEARABLE, TELEVISION
...
class DLPhone(StoreWithUrlExtensions):
    ...
    def products_for_url(cls, url, category=None, extra_args=None):
        ...
        key = soup.find('input', {'name': 'gtm4wp_id'})['value']
        price = Decimal(soup.find('input', {'name': 'gtm4wp_price'})['value'])
        sku = soup.find('input', {'name': 'gtm4wp_sku'})['value']
        picture_urls = [x.find('a')['href'] for x in soup.findAll('div', 'woocommerce-product-gallery__image')]
        description = html_to_markdown(str(soup.find('div', 'woocommerce-product-details__short-description')))
